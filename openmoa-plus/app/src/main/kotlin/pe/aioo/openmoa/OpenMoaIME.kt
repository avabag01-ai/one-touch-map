...
class OpenMoaIME : InputMethodService(), KoinComponent {
    ...
    override fun onCreate() {
        ...
        // Gesture preview receiver
        gestureReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                if (!this@OpenMoaIME::binding.isInitialized) return
                val type = intent.getStringExtra("type") ?: return
                when (type) {
                    "start" -> {
                        gestureKey = intent.getStringExtra("key")
                        lastPreviewMoeum = null
                        if (config.showTopPreview) {
                    ...
                    "move" -> {
                        ...
                        if (previewMoeum != null && previewMoeum.isNotEmpty() && previewMoeum != lastPreviewMoeum) {
                            ...
                            if (config.showTopPreview) {
                                val previewChar = HangulHintData.composeSyllablePublic(key, previewMoeum)
        ...
        LocalBroadcastManager.getInstance(this).registerReceiver(
            gestureReceiver, IntentFilter(GESTURE_ACTION)
        )
    }
    ...
    override fun onDestroy() {
        ...
        if (this::gestureReceiver.isInitialized) {
    ...
    companion object {
        ...
        const val GESTURE_ACTION = "gesturePreview"
    }
